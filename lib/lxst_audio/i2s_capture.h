...
public:
    ...
    /**
     * HYBRID ADDITION: persistent-driver mode. When set, stop() halts
     * the I2S clocks but keeps the driver (and its internal-RAM DMA
     * buffers) installed, so per-call start needs zero internal-heap
     * allocation — the hybrid pre-installs the driver at boot while
     * memory is still contiguous (fragmentation killed call-time
     * installs on device, 2026-07-22).
     */
...
private:
    ...
    bool persistent_ = false;   // hybrid: keep driver installed across calls
