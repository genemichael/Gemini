...
// HYBRID CHANGE (vs upstream pyxis): static task stack. The dynamic
// 24KB xTaskCreate stack failed on-device ("capture start FAILED",
// 2026-07-22) — by call time the launcher's internal heap is too
// fragmented for a 24KB contiguous block. A BSS stack is reserved at
// boot and can never fail. Size stays at upstream's 24KB: a 16KB
// experiment tripped the stack canary ON DEVICE inside the 700C
// encode path (Guru Meditation, lxst_cap, 2026-07-22) — Torlando's
// number was real headroom for codec work, not just UDP logging.
static StaticTask_t s_cap_tcb;
static StackType_t  s_cap_stack[24576 / sizeof(StackType_t)];
...
bool I2SCapture::start() {
    ...
    TaskHandle_t h = xTaskCreateStaticPinnedToCore(
        captureTask, "lxst_cap", sizeof(s_cap_stack) / sizeof(StackType_t),
        this, CAPTURE_TASK_PRIORITY, s_cap_stack, &s_cap_tcb,
        CAPTURE_TASK_CORE);
    taskHandle_ = h;
...
void I2SCapture::stop() {
    ...
    if (i2sInitialized_) {
        i2s_stop(I2S_NUM_1);
        if (!persistent_) {
            // Hybrid: persistent mode keeps the driver + DMA buffers
            // installed across calls (see setPersistent()).
            i2s_driver_uninstall(I2S_NUM_1);
            i2sInitialized_ = false;
        }
    }
...
void I2SCapture::captureLoop() {
...

    {
        ...
        snprintf(logbuf, sizeof(logbuf), "[CAP] Capture task on core %d, I2S=%dHz, codec=%dHz, stack=%d",
                 ...
                 (int)sizeof(s_cap_stack));   // actual static stack, not the constant
        pyxis_log(logbuf);
    }
